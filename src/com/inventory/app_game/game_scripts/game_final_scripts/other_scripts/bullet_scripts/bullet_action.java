...
public abstract class bullet_action {
    ...
    public static void bullet_exit(game_handler handler, game_object operator_object, int mouse_x, int mouse_y) {
        game_object temp_bullet = handler.add_object(
            new bullet(object_tag.bullet, handler, operator_object.getPosition_x(), operator_object.getPosition_y(), 0)
        );


        double angle = Math.atan2(mouse_y - operator_object.getPosition_y(), mouse_x - operator_object.getPosition_x());

        ((bullet) temp_bullet).setBullet_velocity_x(3 * Math.cos(angle));
        ((bullet) temp_bullet).setBullet_velocity_y(3 * Math.sin(angle));
